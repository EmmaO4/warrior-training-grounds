...
def signin():
    ...
    auth_url = _build_msal_app().get_authorization_request_url(
        SCOPE,
        state=session["state"],
        redirect_uri=url_for("callback", _external=True) # match callback to user-initiated session. external url
    )
...
def signout():
    ...
    session.clear() # terminate user session for Grounds app
    # log user from MS
    logout_url = (
...
@app.route("/inactivity_signout")
def inactivity_signout():
    return render_template("inactivity-signout.html", authority=AUTHORITY)
...
def callback():
    ...
    if "id_token_claims" in result:
        ...
        session["last_activity"] = datetime.now(timezone.utc).isoformat() # inactivity counter
...
"""
 from msal library documentation: 
    ClientApplication/ ConfidentialClientApplication(
        ...
        oidc_authority=None)
"""
# creates auth URLs, exchanges auth code for tokens, caches tokens. MSAL client for api requests
...
def check_inactivity():
    if "user" in session:
        ...
        last_activity = session.get("last_activity") # set last activity 

        if last_activity:
            last_activity = datetime.fromisoformat(session["last_activity"]) # update request after every user interaction. reset inactivity timer

            # inactivity logic: if most recent activity - last activity > timeout constant 
            if datetime.now(timezone.utc) - last_activity > SESSION_TIMEOUT: 
                session.clear()

                return redirect(url_for("inactivity_signout"))
            
        session["last_activity"] = datetime.now(timezone.utc).isoformat() # update last activity